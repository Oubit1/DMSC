...
class DownsamplingConvBlock1(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, stride=4, normalization='none'):
        super(DownsamplingConvBlock1, self).__init__()
...
class DownsamplingConvBlock2(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, stride=16, normalization='none'):
...
class Upsampling_function(nn.Module):
    ...
    def fuse_features(self, anchor, positive, negative, attention_weights):
        fused_features = attention_weights[:, 0].unsqueeze(1) * anchor + \
                         attention_weights[:, 1].unsqueeze(1) * positive + \
                         attention_weights[:, 2].unsqueeze(1) * negative
