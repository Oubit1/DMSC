...
def check_hdf5_file(file_path):
    try:
        ...
        print("Unable to open file or file format is not HDF5.")
...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
        ...
        with open('./data/Pancreas_h5/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["./data/Pancreas_h5/" + item.replace('\n', '') + "_norm.h5" for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        # print("image_path",image_path)
        # check_hdf5_file(image_path)
