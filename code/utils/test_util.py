...
def test_all_case1(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
    for image_path in tqdm(image_list):
        ...
        if preproc_fn is not None:
            image = preproc_fn(image)
        prediction, score_map = test_single_case1(net, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
...
def test_single_case1(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    # if the size of image is less than patch_size, then padding it 图像边缘填充，避免因为卷积运算导致输出图像缩小和图像边缘信息丢失
    add_pad = False
    if w < patch_size[0]:
        w_pad = patch_size[0]-w
        add_pad = True
    ...
    dl_pad, dr_pad = d_pad//2,d_pad-d_pad//2
    if add_pad:
        # pad(array, pad_width, mode, **kwargs)array——表示需要填充的数组；pad_width——表示每个轴（axis）边缘需要填充的数值数目。mode——表示填充的方式 constant——表示连续填充相同的值，；
