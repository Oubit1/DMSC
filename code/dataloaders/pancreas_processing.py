...
list_images = sorted(glob('./data/Pancreas-CT/data/*.nii.gz'))
list_labels = sorted(glob('./data/Pancreas-CT/TCIA_pancreas_labels/*.nii.gz'))

for image_path, label_path in tqdm(zip(list_images, list_labels)):
    name_image = str(image_path)
    name_label = label_path.replace('./data/Pancreas-CT/data', './data/Pancreas-CT/TCIA_pancreas_labels').replace('PANCREAS_', 'label')
    name_label = os.path.normpath(name_label.replace('data', '', 1))
    itk_img = sitk.ReadImage(name_image)
    itk_img = ImageResample(itk_img)
    image = sitk.GetArrayFromImage(itk_img)
    image = np.transpose(image, (2, 1, 0))
    
    itk_label = sitk.ReadImage('./data/' + name_label)
    ...
    h5_file = '/home/wangjh/project/CC-Net-V2/data/Pancreas_h5/' + file_name + '_norm.h5'
